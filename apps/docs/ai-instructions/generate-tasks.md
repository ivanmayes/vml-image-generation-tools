...
9.  **Save Task List:** Save the generated document in the `.claude/tasks/` directory with the filename `tasks-[prd-file-name].md`, where `[prd-file-name]` matches the base name of the input PRD file (e.g., if the input was `prd-user-profile-editing.md`, the output is `tasks-prd-user-profile-editing.md`).
...
- Always add a subtask to use apps/docs/developer/css-theming.md when doing front-end tasks to make sure the styling is using the proper variables
...
As you build subtasks, evaluate it for its complexity on a 1-10 system where 1 is a 1-3 line fix and 10 is a complex refactor of 10 files.  Append the complexity to the end of the subtask with this pattern:
[ ] 1.1 [Sub-task description 1.1] (C[INSERT_COMPLEXITY])
